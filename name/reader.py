...
def read_url_execute(uri):
    '''Opens file at specified URL, reads it line by line and calls functions with values found. Called by main()'''
    print("Running...")
    ...
    for line in buffer.split('\n'):
        ...
        line = line.split()
        if len(line) == 0:
            #ie: we've reached the last line
            print("Finished calculating!")
            map_and_count(size, new_map, uri)
            #print("Commands executed:", count)
        elif len(line) == 1:
                #i.e.: the only line with one value is the first, the line with the grid size.
                if 0 >= int(line[0]) > 999999:
            ...
            if line[1] == "on":
                x1, y1, x2, y2 = int(line[2]), int(line[3]),int(line[5]), int(line[6])
                if x2 >= x1 and y2 >= y1:
            ...
            x1, y1, x2, y2 = int(line[1]), int(line[2]), int(line[4]), int(line[5])
            if x2 >= x1 and y2 >= y1:
                switch(x1, y1, x2, y2, size, new_map)
                #count += 1

def read_url_count(uri):
    '''Opens file at specified URL,counts lines in file and prints number. For testing.'''
    req = urllib.request.urlopen(uri)
...
def read_url_commands(uri):
    '''Opens file at specified URL, prints command and coordinates at each line. For testing.'''
...
def read_file(filename):
    '''Opens file at specified local address, prints command at each line. For testing.'''
...
def read_file_execute(filename):
    '''Opens file at specified local address, reads it line by line and calls functions with values found'''
    buffer = open(filename).read()
    for line in buffer.split('\n'):
        if len(line.split()) == 0:
            print("Input ended")
...
def map_and_count(size, new_map, uri):
    '''Output. Printed grid, name of file executed and number of lights switched on'''
    '''
